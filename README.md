...
- **Сетки.** `buildReferences()` заполняет `#references-grid` карточками
  ...
  категория + текстовый поиск + теги); `buildResourceSection(id, items)` —
...
> дописывает к ней активные фильтры (категория + текст поиска + выбранные
> теги), так что запрос отражает то, что отфильтровано на экране. Фильтр
> платформы выбирает, по каким площадкам показывать ссылки.
